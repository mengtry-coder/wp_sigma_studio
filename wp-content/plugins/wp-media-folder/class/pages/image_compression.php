...
if (isset($_GET['action'])) {
    ...
    if ('install-plugin' == $action) {
        ...
        if (isset($_GET['from']) && 'import' == $_GET['from']) {
            ...
            wp_redirect(self_admin_url("options-general.php?page=option-folder")); // overrides the ?error=true one above
        }
    }
}
$button = '';
...
if (!file_exists(WP_PLUGIN_DIR . '/imagerecycle-pdf-image-compression')) {
    ...
    if (!is_plugin_active($plugin_file)) {
        $url = wp_nonce_url('options-general.php?page=option-folder&action=activate&amp;plugin=' . $plugin_file, 'activate-plugin_' . $plugin_file);
        if (is_multisite()) {
            if (current_user_can('manage_network_plugins')) {
                $button = '<a style="float: right; background: #554766; font-size: 12px; line-height: 18px; text-align: center;  color: #fff;font-size: 14px;text-decoration: none; text-transform: uppercase; padding: 5px 20px; font-weight:bold;" href="' . $url . '" class="edit" aria-label="Activate ImageRecycle pdf &amp; image compression">' . __('Activate Plugin', 'wpmf') . '</a>';
            }
        } else {
            $button = '<a style="float: right; background: #554766; font-size: 12px; line-height: 18px; text-align: center;color: #fff;font-size: 14px;text-decoration: none; text-transform: uppercase; padding: 5px 20px; font-weight:bold;" href="' . $url . '" class="edit" aria-label="Activate ImageRecycle pdf &amp; image compression">' . __('Activate Plugin', 'wpmf') . '</a>';
        }
...
<div class="main-presentation" style="margin: 20px auto;margin-top: 10px ;max-width: 1200px;font-family: helvetica,arial,sans-serif;">
...
<?php else: ?>
        ...
        <div class="main-presentation" style="margin: 0px auto;margin-top: 10px;max-width: 1200px; background-color:#fff;font-family: helvetica,arial,sans-serif;">
            <div class="main-textcontent" style="margin: 0px auto; min-height: 400px; border-left: 1px dotted #d2d3d5; border-right: 1px dotted #d2d3d5; width: 840px; background-color:#fff;border-top: 5px solid #544766;" cellspacing="0" cellpadding="0" align="center">
                ...
                <div style="background-color:#fafafa; border: 2px dashed #ccc; border-left: 5px solid #A1B660; font-size: 30px; padding: 20px; line-height: 40px;">ImageRecycle 20% OFF, apply on all memberships: WPMF-20</div>
                <p style="background-color: #ffffff; color: #445566; font-family: helvetica,arial,sans-serif; font-size: 12px; line-height: 22px; padding-left: 20px; padding-right: 20px; text-align: center;  font-style: italic;">ImageRecycle got a dedicated plugin for WordPress that run the images and PDF optimization automatically on your website.
                    <br>In order to start the optimization process, please install the WordPress plugin. Enjoy!</p>
                <p></p>
                <p><?php echo $button ?></p>
            </div>
