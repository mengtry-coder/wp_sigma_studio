...
class Wpmf_watermark{
    ...
    function __construct() {
        ...
        add_filter( 'wp_generate_attachment_metadata', array($this,'wpmf_create_waltermark_image'), 10, 2);
    }

    public function wpmf_create_waltermark_image($metadata, $attachment_id){
        $option_image_watermark = get_option('wpmf_option_image_watermark');
        $post_upload = get_post($attachment_id);
        if(empty($option_image_watermark) || (isset($post_upload->post_mime_type) && strpos($post_upload->post_mime_type,'image') === false)){
            return $metadata;
        }
        if(!empty($attachment_id)){
            $check_remote = get_post($attachment_id);
            if(!empty($check_remote) && $check_remote->post_content == 'wpmf_remote_video') return $metadata;
    ...
    public function regeneratePictures(){
        ...
        $sql = $wpdb->prepare("SELECT COUNT(ID) FROM " . $wpdb->posts . " WHERE  post_type = 'attachment' AND ID != %d AND post_mime_type LIKE %s AND guid  NOT LIKE %s AND post_content != %s",array($logo_image_id,'image%','%.svg','wpmf_remote_video'));
        $count_images = $wpdb->get_var($sql);
        $present = (100 / $count_images) * $limit;
        $k = 0;
        $sql = $wpdb->prepare("SELECT ID FROM " . $wpdb->posts . " WHERE  post_type = 'attachment' AND ID != %d AND post_mime_type LIKE %s AND guid NOT LIKE %s AND post_content != %s LIMIT %d OFFSET %d",array($logo_image_id,'image%','%.svg','wpmf_remote_video',$limit,$ofset));
    ...
    public function generatePicture($newname,$imageInfo,$full_dir){
        ...
        if($imageInfo['mime']=='image/jpeg'){
            if (!empty($wtm_images) && $check_image_logo_exit){
                $this->check_copyFileWithNewName($full_dir,$newname,$wtm_apply_on);
            }
    ...
    public function check_copyFileWithNewName($full_dir,$newname,$wtm_apply_on){
