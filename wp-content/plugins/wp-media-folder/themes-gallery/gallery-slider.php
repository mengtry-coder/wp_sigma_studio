...
foreach ($attachments as $id => $attachment) {
    ...
    if (!empty($link)) {
        if ($customlink) {
            ...
            $image_output = '<a class="' . $tclass . '" href="' . $url . '" target="' . $link_target . '">' . $image_output . '</a>';
        } else if ('post' === $link) {
            $url = get_attachment_link($id);
            $image_output = '<a class="' . $tclass . '" href="' . $url . '" target="' . $link_target . '">' . $image_output . '</a>';
        } else if ('file' === $link) {
            ...
            if(!empty($remote_video)){
                $image_output = '<a class="' . $tclass . ' isvideo" data-lightbox="' . $lightbox . '" href="' . $remote_video . '" target="' . $link_target . '" title="' . $post_title . '">' . $image_output . '</a>';
            }else{
                $image_output = '<a class="' . $tclass . ' not_video" data-lightbox="' . $lightbox . '" href="' . $url . '" target="' . $link_target . '" title="' . $post_title . '">' . $image_output . '</a>';
            }
            ...
            if (get_post_meta($id, _WPMF_GALLERY_PREFIX . 'custom_image_link', true) != '') {
                $lightbox = 0;
                $url = get_post_meta($id, _WPMF_GALLERY_PREFIX . 'custom_image_link', true);
                $image_output = '<a class="' . $tclass . '" data-lightbox="' . $lightbox . '" href="' . $url . '" target="' . $link_target . '" title="' . $post_title . '">' . $image_output . '</a>';
            } else {
                if ($columns == 1) {
                    $image_output = "<img src='{$src}' width='{$width}' height='{$height}' alt='{$alt}' />";
                } else {
    ...
    if ($columns == 1) {
        $output .= "<li class='wpmf-gg-one-columns wpmf-gallery-item item wpmf-gallery-item-position-" . $pos . " wpmf-gallery-item-attachment-" . $id . "'>";
    } else {
        $output .= "<li class='wpmf-gallery-item item wpmf-gallery-item-position-" . $pos . " wpmf-gallery-item-attachment-" . $id . "' style='min-height:0px;position: relative;height:" . max($height_array) . "px'>";
    }
